...
-- Garantir consistência: tipo e nome sempre juntos; ref_id é opcional (ex: 'estudio' não tem ref)
...
ALTER TABLE kit_itens ADD CONSTRAINT check_localizacao_complete CHECK (
    (localizacao_tipo IS NULL AND localizacao_nome IS NULL)
    OR
    (localizacao_tipo IS NOT NULL AND localizacao_nome IS NOT NULL)
);
