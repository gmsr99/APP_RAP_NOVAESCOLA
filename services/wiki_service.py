        ...
        for disc in disc_rows:
            ...
            cur.execute("""
                SELECT uuid, nome, codigo, sessoes_previstas,
                       horas_por_sessao, musicas_previstas, perfil_mentor
...
def criar_disciplina_turma(turma_id: int, nome: str, descricao: str = None, musicas_previstas: int = 0, atividades: list = None):
    ...
    atividades = [{ nome, codigo, sessoes_previstas, horas_por_sessao, musicas_previstas, perfil_mentor }]
    """
    try:
        conn = get_db_connection()
        cur = conn.cursor()
...
def criar_atividade(turma_disciplina_id: int, nome: str, codigo: str = None, sessoes_previstas: int = 0, horas_por_sessao: float = 0, musicas_previstas: int = 0, perfil_mentor: str = None):
    ...
    try:
        ...
        uuid = cur.fetchone()[0]
        conn.commit()
        return {'uuid': str(uuid), 'nome': nome, 'codigo': codigo}
    except Exception as e:
