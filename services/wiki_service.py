                    ...
                    if has_auto:
                        cur.execute("""
                            SELECT
                                ...
                                ta.horas_por_sessao, ta.musicas_previstas, ta.role,
            ...
            if has_auto:
                cur.execute("""
                    SELECT uuid, nome, codigo, sessoes_previstas,
                           horas_por_sessao, musicas_previstas, role, is_autonomous
                ...
                cur.execute("""
                    SELECT uuid, nome, codigo, sessoes_previstas,
                           horas_por_sessao, musicas_previstas, role, FALSE AS is_autonomous
                    FROM turma_atividades
                    WHERE turma_disciplina_id = %s
...
def criar_disciplina_turma(turma_id: int, nome: str, descricao: str = None, musicas_previstas: int = 0, atividades: list = None):
    ...
    atividades = [{ nome, codigo, sessoes_previstas, horas_por_sessao, musicas_previstas, role, is_autonomous }]
    """
    try:
        conn = get_db_connection()
        cur = conn.cursor()
                ...
                if has_auto:
                    cur.execute("""
                        INSERT INTO turma_atividades
                            (turma_disciplina_id, nome, codigo, sessoes_previstas, horas_por_sessao, musicas_previstas, role, is_autonomous)
                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                        RETURNING uuid
                    """, (
                        td_id, a['nome'], a.get('codigo'),
                        a.get('sessoes_previstas', 0), a.get('horas_por_sessao', 0),
                        a.get('musicas_previstas', 0), a.get('role') or None,
                        bool(a.get('is_autonomous', False))
                    ))
                else:
                    cur.execute("""
                        INSERT INTO turma_atividades
                            (turma_disciplina_id, nome, codigo, sessoes_previstas, horas_por_sessao, musicas_previstas, role)
...
def criar_atividade(turma_disciplina_id: int, nome: str, codigo: str = None, sessoes_previstas: int = 0, horas_por_sessao: float = 0, musicas_previstas: int = 0, role: str = None, is_autonomous: bool = False):
    ...
    try:
        ...
        if has_auto:
            cur.execute("""
                ...
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                RETURNING uuid
            """, (turma_disciplina_id, nome, codigo, sessoes_previstas, horas_por_sessao, musicas_previstas, role or None, is_autonomous))
        else:
            cur.execute("""
                INSERT INTO turma_atividades
                    (turma_disciplina_id, nome, codigo, sessoes_previstas, horas_por_sessao, musicas_previstas, role)
                VALUES (%s, %s, %s, %s, %s, %s, %s)
                RETURNING uuid
            """, (turma_disciplina_id, nome, codigo, sessoes_previstas, horas_por_sessao, musicas_previstas, role or None))
        uuid = cur.fetchone()[0]
        conn.commit()
        return {'uuid': str(uuid), 'nome': nome, 'codigo': codigo, 'role': role, 'is_autonomous': is_autonomous}
    except Exception as e:
        ...
        if has_auto:
            cur.execute("""
                ...
                SET nome = %s, codigo = %s, sessoes_previstas = %s,
                    horas_por_sessao = %s, musicas_previstas = %s, role = %s,
                    is_autonomous = %s
                WHERE uuid = %s
            """, (
                dados['nome'], dados.get('codigo'),
                dados.get('sessoes_previstas', 0), dados.get('horas_por_sessao', 0),
                dados.get('musicas_previstas', 0), dados.get('role') or None,
